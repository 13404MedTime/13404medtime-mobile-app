...
class UpcomingVisitsBloc
    extends Bloc<UpcomingVisitsEvent, UpcomingVisitsState> {
  UpcomingVisitsBloc(this.homeRepository) : super(const UpcomingVisitsState()) {
    on<UpcomingVisitsEvent>((event, emit) => switch (event) {
          GetDoctorRequests$UpcomingVisitsEvent _ =>
            _onGetDoctorRequests(event, emit),
          ChangeSelected$UpcomingVisitsEvent _ =>
            _onChangeSelected(event, emit),
        });
  }
  ...
  Future<void> _onGetDoctorRequests(
    ...
    emit(state.copyWith(
      bookings: Map.fromEntries(
        map.entries.toList()
          ..sort(
            (a, b) => a.value.first.doctorBookingIdData.date
                .compareTo(b.value.first.doctorBookingIdData.date),
          ),
      ),
