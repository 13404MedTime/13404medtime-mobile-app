...
class MyVisitBloc extends Bloc<MyVisitEvent, MyVisitState> {
    ...
    response.fold(
      ...
      (r) {
        if (r.data?.data?.response?.isNotEmpty ?? false) {
          final ConsultationDaysInfo? subscription =
              r.data?.data?.response?.first;
          ...
          emit(state.copyWith(
            ...
            isExistConsultationDays:
                (subscription?.consultationCount == null) ? false : true,
          ));
        } else {
  ...
  Future<void> _bookDoctor(
      BookDoctorEvent event, Emitter<MyVisitState> emit) async {
    emit(state.copyWith(isLoading: true));
    try {
      ...
      _getSelectedDate(
        GetSelectedDateEvent(
          selectedDate:
              state.selected?.date.subtract(const Duration(days: 1)) ??
                  DateTime.now(),
        ),
        emit,
      );
  ...
  Future<void> _getDoctorBookingRequests(
    ...
    response.fold(
      ...
      (r) {
        emit(state.copyWith(
          bookingRequests: r.data.data.response
              .where((e) => e.clientsId == localSource.userId)
              .toList(),
        ));
      },
    );
