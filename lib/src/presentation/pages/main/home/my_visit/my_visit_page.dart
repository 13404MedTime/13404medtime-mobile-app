...
class _MyVisitPageState extends State<MyVisitPage>
    with MyVisitMixin, RemoteLikeMixin {
  ...
  Widget build(BuildContext context) {
    final FavouriteDoctorBloc? selectedDoctorsBloc =
        widget.arguments?.favoriteBloc;
    final experYear = (_bloc.state.visitData?.doctorIdData ??
                (isHavePurpose
                    ? myVisit?.doctorIdData
                    : widget.arguments?.doctor))
            ?.experience ??
        0;
    final String guid = (_bloc.state.visitData?.doctorIdData ??
                (isHavePurpose
                    ? myVisit?.doctorIdData
                    : widget.arguments?.doctor))
            ?.guid ??
    ...
    return Scaffold(
              ...
              child: InkWell(
                ...
                child: Padding(
                  ...
                  child: Icon(
                    ...
                    size: 24,
                    color: (box.get(AppKeys.favouriteDoctors)?.contains(guid) ??
                            false)
      ...
      body: BlocBuilder<MyVisitBloc, MyVisitState>(
        builder: (context, state) {
          final DoctorIdData? doctor = state.visitData?.doctorIdData ??
              (isHavePurpose
                  ? myVisit?.doctorIdData
                  : widget.arguments?.doctor);
          late final MyVisitModel? myVisitData =
              state.visitData ?? widget.arguments?.myVisit;
                  ...
                  onRefresh: () async {
                    context.read<MyVisitBloc>()
                      ..add(GetDoctorFreeTimeEvent(
                          guid: guid, date: selectedDate.value))
                                  ...
                                  Column(
                                    children: [
                                      Wrap(
                                        ...
                                        children: [
                                          for (final i
                                              in state.selectedBookings)
                                            InkWell(
                                              onTap: () {
                                                if (!i.isBooked &&
                                                    !state.bookingRequests.any((e) =>
                                                        e.doctorBookingIdData
                                                                .date ==
                                                            i.date
                                                                .toIso8601String() &&
                                                        e.doctorBookingIdData
                                                          ...
                                                          color: i.isBooked ||
                                                                  state.bookingRequests.any((e) =>
                                                                      ...
                                                                      e.doctorBookingId ==
                                                                          i
                                                                              .guid &&
                                                                      e.doctorBookingIdData.fromTime ==
                                                                          i
                                                                              .fromTime &&
                                                                      e.doctorBookingIdData.toTime ==
                                                                          i
                                                                              .toTime)
                                                              ? context
                                                                  .colorScheme
                        ...
                        if (isHavePurpose) ...[
                          ...
                          PurposeWidget(
                            ...
                            numberMedicationsTaken:
                                myVisitData?.amountMedicationTaken ?? 0,
                            onTap: () {
                              Navigator.pushNamed(
                                context,
                                Routes.purposePage,
