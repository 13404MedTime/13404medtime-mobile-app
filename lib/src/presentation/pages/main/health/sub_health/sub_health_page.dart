...
class _SubHealthPageState extends State<SubHealthPage> with SubHealthMixin, TickerProviderStateMixin {
  bool isUpdate = false;
  ...
  Widget build(BuildContext context) => BlocConsumer<HealthBloc, HealthState>(
        ...
        builder: (_, state) => Scaffold(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    CustomTabBarWidget(
                      onTap: (index) => _onTabBarTap(index, _getChangedList(state)),
          ...
          bottomNavigationBar: Visibility(
            visible: (widget.args.type != SubHealthType.pedometer && Platform.isIOS) || Platform.isAndroid,
            child: DecoratedBox(
                  ...
                  onPressed: () async {
                    ...
                    switch (widget.args.type) {
                      case SubHealthType.pedometer:
                        result = await customModalBottomSheet<SubHealthBottomSheetArgs?>(
                          context: context,
                          builder: (_, controller) => const PedometerBottomSheetWidget(),
                        );
                      case SubHealthType.arterialPressure:
                        if (!mounted) return;
                        result = await customModalBottomSheet<SubHealthBottomSheetArgs?>(
                          context: context,
                          builder: (_, controller) => const PressureBottomSheetWidget(),
                        );
                      case SubHealthType.bloodSugar:
                        ...
                        result = await customModalBottomSheet<SubHealthBottomSheetArgs?>(
                          context: context,
                          builder: (_, controller) => const BloodSugarMainBottomSheet(),
                        );
                      case SubHealthType.twh:
                        if (!mounted) return;
                        result = await customModalBottomSheet<SubHealthBottomSheetArgs?>(
                          context: context,
                          builder: (_, controller) => const TemperatureWeightHeightBottomSheet(),
                        );
                    }
                    if (result is SubHealthBottomSheetArgs) {
                      ...
                      if (widget.args.type == SubHealthType.pedometer) {
                        widget.args.bloc.add(const UpdatedPedometerIsUpdatedEvent(isUpdate: true));
                      } else if (widget.args.type == SubHealthType.arterialPressure) {
                        widget.args.bloc.add(
